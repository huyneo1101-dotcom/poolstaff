...
App tĩnh một-file: toàn bộ UI + logic + CSS trong `index.html` (~308KB, ~4.575 dòng — **RẤT LỚN**), React 18 + Supabase 2 + QRCode + Tabler icons qua CDN, JSX dịch sẵn lúc dựng (đo 23/08/2026: `index.html` không nạp `@babel/standalone`). Khác các app khác trong hệ sinh thái: đây là app **B2B đa người dùng thật** (nhiều vai + app khách), xử lý **dữ liệu khách hàng thật** (tên, SĐT, đơn, booking).
...
- **CDN đang pin LỎNG (chỉ major)**: `react@18`, `react-dom@18`, `@supabase/supabase-js@2`, `qrcode@1` — CDN nhả bản mới trong major có thể gây trắng màn hình bất ngờ. Nên **pin đúng version** như các app anh em (skill `pwa-healthcheck`).
